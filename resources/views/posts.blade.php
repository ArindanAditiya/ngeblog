  <x-layout :title="$title">    
    {{-- @foreach ( $posts as $post )
      ...
      <div class="text-base text-gray-500">
        By 
        <a href="/authors/{{ $post->author->username }}" class="hover:underline text-gray-900 font-bold" >{{ $post->author->name }}</a>
        In
        <a href="/category/{{ $post->category->slug }}" class="hover:underline text-gray-900 font-bold" >{{ $post->category->name }}</a>
