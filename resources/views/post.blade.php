<x-layout :title="$title">    
    {{-- <article class="py-8 max-w-3xl border-b border-gray-300">
      ...
      <div class="text-base text-gray-500">
        <a href="" class="hover:underline"><b></b></a> | 
      </div>
          ...
          <header class="my-4 lg:mb-6 not-format">
              <address class="flex items-center mb-6 not-italic">
                  <div class="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white">
                      ...
                      <div>
                          <a href="/authors/{{ $post->author->username }}" rel="author" class="block text-xl font-bold text-gray-900 dark:text-white">{{ $post->author->name }}</a>
                          <x-category-link href="/category/{{ $post->category->slug }}" :categorySlug="$post->category->slug">
                            {{ $post->category->name }}
                          </x-category-link>
